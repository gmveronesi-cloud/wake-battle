...
check("lun: niente foto prima del Fatto (flag)", rpc("get_today")["io"]["ha_foto"], False)
check("lun: niente foto prima del Fatto (RPC)", rpc("get_object_photos")["foto"], None)
...
t = rpc("get_today")
check("lun: A vede il flag subito (giornata non chiusa)", (t["chiuso"], t["io"]["ha_foto"]), (False, True))
check("lun: A scarica le sue foto con la RPC", rpc("get_object_photos")["foto"], FA)
check("lun: foto del partner nascoste (giornata non chiusa, flag)", t["partner"]["ha_foto"], False)
check("lun: foto del partner non scaricabili (giornata non chiusa)", rpc("get_object_photos", True)["error"], "non_visibile")
check("lun: sovrascrive, non accumula", jrpc("save_object_photos", foto(2, "z"))["ok"], True)
check("lun: get_object_photos mostra solo l'ultimo salvataggio", rpc("get_object_photos")["foto"], foto(2, "z"))
...
check("lun: B vede il proprio flag", t["io"]["ha_foto"], True)
check("lun: B scarica le proprie foto", rpc("get_object_photos")["foto"], FB)
check("lun: B vede ora anche il flag di A", t["partner"]["ha_foto"], True)
check("lun: B scarica ora anche le foto di A", rpc("get_object_photos", True)["foto"], FA)
as_user(A)
check("lun: A vede ora anche il flag di B", rpc("get_today")["partner"]["ha_foto"], True)
check("lun: A scarica ora anche le foto di B", rpc("get_object_photos", True)["foto"], FB)
...
check("22/09 appena dopo mezzanotte: le foto di lunedì non sono più 'oggi' (flag)", rpc("get_today")["io"]["ha_foto"], False)
check("22/09 appena dopo mezzanotte: la RPC non le restituisce più", rpc("get_object_photos")["foto"], None)
admin()
