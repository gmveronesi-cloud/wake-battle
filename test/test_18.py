...
check("lun: niente video prima del Fatto", rpc("get_today")["io"]["video"], None)
...
t = rpc("get_today")
check("lun: A vede il suo video subito (giornata non chiusa)", (t["chiuso"], t["io"]["video"]), (False, VA))
check("lun: video del partner nascosto (giornata non chiusa)", t["partner"]["video"], None)
check("lun: sovrascrive, non accumula", rpc("save_eye_video", video("z"))["ok"], True)
check("lun: get_today mostra solo l'ultimo salvataggio", rpc("get_today")["io"]["video"], video("z"))
...
check("lun: B vede il suo video", t["io"]["video"], VB)
check("lun: B vede ora anche quello di A", t["partner"]["video"], VA)
as_user(A)
check("lun: A vede ora anche quello di B", rpc("get_today")["partner"]["video"], VB)
...
check("22/09 appena dopo mezzanotte: il video di lunedì non è più 'oggi'", rpc("get_today")["io"]["video"], None)
admin()
