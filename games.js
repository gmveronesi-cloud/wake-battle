...
(function () {
  ...
  // ---------------------------------------------------------------------
  // TROVA L'INTRUSO (v8): 5 turni. Griglia 3×3 di 9 emoji tutte uguali
  // tranne una: resta visibile mostra_ms (come Memoria), poi si copre e
  // si tocca a memoria la casella dell'intrusa. Turno riuscito → il
  // successivo parte da solo. Errore → si riparte dal turno 1 con
  // emoji/posizioni nuove (come Colore della parola).
  // Parametri compatti: ogni turno è [base, intruso, posizione], non la
  // griglia intera; la griglia si ricostruisce qui (tutte = base, tranne
  // "posizione" = intruso).
  // Risposta: { tentativo, risposte: [5 posizioni], tentativi }.
  // ---------------------------------------------------------------------
  function intruso(box, params, opts) {
    const seqs = params.sequenze || [];
    const turns = Number(params.turni) || 5;
    const showMs = Number(params.mostra_ms) || 3000;
    let next = 0;    // prossimo tentativo da usare
    ...
    function intro() {
      box.replaceChildren();
      box.dataset.phase = 'intro';
      box.appendChild(el('p', turns + ' turni: memorizza qual è l\'emoji diversa dalle altre 8 (visibile ' +
        Math.round(showMs / 1000) + ' secondi), poi toccala a memoria. Un errore fa ripartire dal turno 1.', 'hint'));
      const b = el('button', 'Inizia', 'game-start');
      b.type = 'button';
      b.addEventListener('click', newGame);
      box.appendChild(b);
    ...
    // pallini dei turni: pieni = fatti, bordato = in corso
    function dots(turn) {
      const d = el('div', null, 'int-dots');
      for (let i = 0; i < turns; i++) d.appendChild(el('span', null, i < turn ? 'int-ok' : i === turn ? 'int-now' : ''));
      return d;
    }

    // nuovo tentativo dal turno 1, con la prossima sequenza libera
    function newGame() {
      clear();
      if (next >= seqs.length) {
        box.replaceChildren(el('p', 'Sequenze finite.', 'game-note bad'));
    ...
    // fase 1: guarda la griglia
    function look(turn, answers) {
      clear();
      const t = seqs[k][turn];
      box.replaceChildren();
      box.dataset.phase = 'guarda';
      box.dataset.turno = String(turn + 1);
      box.dataset.tentativo = String(tries);
      box.appendChild(dots(turn));
      box.appendChild(el('p', 'Turno ' + (turn + 1) + ' di ' + turns + ' · memorizza!' +
        (tries > 1 ? ' (tentativo ' + tries + ')' : ''), 'game-note'));
      const [base, sym, pos] = t;
      const grid = el('div', null, 'int-grid');
      for (let i = 0; i < 9; i++) grid.appendChild(el('div', SYM[i === pos ? sym : base], 'int-cell'));
      box.appendChild(grid);
      const bar = el('div', null, 'bar game-bar');
      const fill = el('div');
      bar.appendChild(fill);
      ...
      fill.style.transition = 'width ' + showMs + 'ms linear';
      requestAnimationFrame(() => requestAnimationFrame(() => { fill.style.width = '0%'; }));
      later(() => answer(turn, answers, t), showMs);
    }

    // fase 2: tocca l'intrusa a memoria
    function answer(turn, answers, t) {
      box.replaceChildren();
      box.dataset.phase = 'tocca';
      box.appendChild(dots(turn));
      const note = el('p', 'Turno ' + (turn + 1) + ' di ' + turns + ' · tocca l\'intrusa', 'game-note');
      box.appendChild(note);
      const pos = t[2];
      const grid = el('div', null, 'int-grid');
      const cells = [];
      for (let i = 0; i < 9; i++) {
        const c = el('button', '', 'int-cell int-copri');
        c.type = 'button';
      ...
      cells.forEach((c, i) => c.addEventListener('click', () => {
        if (box.dataset.phase !== 'tocca') return;
        cells.forEach((x) => { x.disabled = true; });
        if (i === pos) {
          const doneAnswers = answers.concat([i]);
          const nextTurn = turn + 1;
          if (nextTurn >= turns) {
            note.textContent = 'Controllo…';
            box.dataset.phase = 'fine';
            opts.onDone({ tentativo: k, risposte: doneAnswers, tentativi: tries });
          } else {
            note.textContent = '✓ Turno ' + nextTurn + ' fatto!';
            note.className = 'game-note good';
            box.dataset.phase = 'ok';
            later(() => look(nextTurn, doneAnswers), 700);
          }
        } else {
    ...
    return {
      destroy() { dead = true; clear(); box.replaceChildren(); delete box.dataset.phase; },
      // il server ha rifiutato la risposta: nuovo tentativo con emoji nuove
      retry() { if (!dead) { clear(); newGame(); } },
    };
  }

  const GAMES = { memoria, numeri, colore_parola: coloreParola, intruso };
