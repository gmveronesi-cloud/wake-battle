...
- Accendi la luce (definitiva dal 23/09/2026, da verificare sul telefono, se inaffidabile si toglie): primo gioco con fotocamera. Si chiede il permesso della fotocamera POSTERIORE; niente feed video in diretta a schermo, solo una barra col livello di luminosità rilevato in tempo reale. "Accesa" = media dei toni di grigio del frame (scala 0-255) sopra soglia fissa 120, per almeno 5 frame consecutivi (evita falsi positivi da un singolo flash); appena rilevata la risposta parte da sola. Nessun timeout dedicato: vale il limite generale di 5 minuti dalla sveglia (regola 2). Permesso negato o fotocamera non disponibile → messaggio con pulsante "Riprova". Il server non riceve né verifica dati reali del sensore (non può controllare cosa inquadra davvero la fotocamera): si fida della conferma del client. Server: params {gioco:'luce'} (nessun dato da generare), risposta {fatto:true} accettata se il campo è booleano true. La parte generica (permesso, video nascosto, ciclo di cattura frame) è scritta in games.js come funzioni riutilizzabili dai prossimi giochi con fotocamera (QR, Caccia ai colori, Occhi aperti, Trova l'oggetto): cambia solo la funzione di analisi del frame.
